...
import { EFFORT_HINT } from './theme.js';
const MODELS = [
...
export async function runWizard() {
    ...
    const summary = `${chalk.dim('model:')} ${chalk.whiteBright(result.model.split('/').pop())}  ${chalk.dim('•')}  ${chalk.dim('effort:')} ${chalk.whiteBright(result.effort)}  ${chalk.dim(`— ${EFFORT_HINT[result.effort]}`)}`;
